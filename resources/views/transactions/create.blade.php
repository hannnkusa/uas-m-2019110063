      ...
      <div class="row gx-4">
        <div class="col-auto">
          <div class="avatar avatar-xl position-relative">
            <img src="../assets/img/small-logos/logo-spotify.svg" alt="..." class="w-100 border-radius-lg shadow-sm">
            <a href="javascript:;" class="btn btn-sm btn-icon-only bg-gradient-light position-absolute bottom-0 end-0 mb-n2 me-n2">
              <i class="fa fa-pen top-0" data-bs-toggle="tooltip" data-bs-placement="top" title="Just Decoration"></i>
            </a>
          </div>
        </div>
        <div class="col-auto my-auto">
          <div class="h-100">
            ...
            <p class="mb-0 font-weight-bold text-sm">
              {{ __('this form used to be create new transaction') }}
            </p>
          </div>
        </div>
  ...
  <div class="container-fluid py-4">
    <div class="card">
      ...
      <div class="card-body pt-4 p-3">
        <form action="{{ route('transactions.store') }}" method="POST" role="form text-left">
          @csrf
          @if($errors->any())
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label for="jenis" class="form-control-label">{{ __('Kategori') }}</label>
                <div class="@error('kategori')border border-danger rounded-3 @enderror">
                  <select class="form-select" value="{{ old('kategori') }}" placeholder="Pilih Kategori" id="kategori" name="kategori">
                    <option>Music</option>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="name" class="form-control-label">{{ __('Nominal') }}</label>
                <div class="@error('nominal')border border-danger rounded-3 @enderror">
                  <input class="form-control" value="{{ old('nominal') }}" type="number" placeholder="Nominal" id="nominal" name="nominal">
                  @error('nominal')
                  <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="name" class="form-control-label">{{ __('Penerima Pembayaran') }}</label>
                <div class="@error('Penerima Pembayaran')border border-danger rounded-3 @enderror">
                  <select class="form-select" value="{{ old('tujuan') }}" type="number" placeholder="Penerima Pembayaran" id="Penerima Pembayaran" name="tujuan">
                    @foreach ($accounts as $item)
                    <option value="{{ $item->nama }}">{{ $item->id . ' - ' .$item->nama }}</option>
                    @endforeach
                  </select>
                  @error('Penerima Pembayaran')
                  <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
              </div>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="name" class="form-control-label">{{ __('Pembeli') }}</label>
                <div class="@error('Pembeli')border border-danger rounded-3 @enderror">
                  <select class="form-select" value="{{ old('account_id') }}" type="number" placeholder="Pembeli" id="Pembeli" name="account_id">
                    @foreach ($accounts as $item)
